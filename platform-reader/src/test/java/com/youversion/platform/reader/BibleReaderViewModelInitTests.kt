...
class BibleReaderViewModelInitTests {
    ...
    fun `init restores saved line spacing`() =
        runTest(testDispatcher) {
            every { userSettingsRepository.readerLineSpacingFraction } returns 0.6f

            val vm = createViewModel()

            assertEquals(0.6f, vm.state.value.lineSpacingFraction)
        }
    ...
    fun `init snaps out-of-range saved line spacing to the nearest supported step`() =
        runTest(testDispatcher) {
            ...
            every { userSettingsRepository.readerLineSpacingFraction } returns 2.5f
    ...
    fun `init snaps between-step saved line spacing to the nearest supported step`() =
        runTest(testDispatcher) {
            ...
            every { userSettingsRepository.readerLineSpacingFraction } returns 0.45f

            val vm = createViewModel()

            assertEquals(0.4f, vm.state.value.lineSpacingFraction)
        }
    ...
    fun `init uses defaults when all settings are null`() =
        runTest(testDispatcher) {
            ...
            assertEquals(ReaderFontSettings.DEFAULT_LINE_SPACING_FRACTION, vm.state.value.lineSpacingFraction)
        }
